  ...
  const handlePlayheadDrag = useCallback(
    (e: MouseEvent) => {
      ...
      const clampedFrameIndex = Math.max(
        0,
        Math.min(videoFrames.length - 1, targetFrameIndex)
      );

      onFrameSelect(clampedFrameIndex);
    },
    [
      isDraggingPlayhead,
                    ...
                    {videoFrames.map((frameUrl, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* 프레임 번호 - 항상 표시 (최상위) */}
                          <div
                            className={`absolute inset-x-0 bottom-0 bg-black/70 text-white text-center py-0.5 z-10 ${
                              frameWidth > 40 ? "text-xs" : "text-[10px]"
                            }`}
                          >
                            {index + 1}
                          </div>
